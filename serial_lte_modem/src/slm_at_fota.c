			 ...
			 uint8_t pdn_id, enum dfu_target_image_type type)
{
	...
	if (slm_util_casecmp(schema, SCHEMA_HTTPS)) {
		...
		rsp_send("\r\n001 hostname:%s, path:%s, sec_tag:%d, pdn_id:%d, type:%d\r\n", hostname, path, sec_tag, pdn_id, type);
		ret = fota_download_start_with_image_type(hostname, path, sec_tag, pdn_id, 0, type);
	} else if (slm_util_casecmp(schema, SCHEMA_HTTP)) {
		rsp_send("\r\n002 hostname:%s, path:%s, pdn_id:%d, type:%d\r\n", hostname,path, pdn_id, type);
	...
	case AT_CMD_TYPE_SET_COMMAND:
		...
		case SLM_FOTA_START_FULL_FOTA:
			...
			if (param_count > 4) {
				...
				rsp_send("\r\n001 uri:%s, sec_tag:%d, pdn_id:%d, type:%d\r\n", uri,sec_tag,pdn_id,type);	
				err = do_fota_start(op, uri, sec_tag, pdn_id, type);
			} else {
				rsp_send("\r\n002 uri:%s, sec_tag:%d, type:%d\r\n", uri,sec_tag,type);	
